...
let sphere;   
let showGrid;
let arrows = [];                        // Central sphere object
...
document.getElementById('gui-container').appendChild(gui.domElement);
    ...
    // Add the showInfo function to the params object
    params.showInfo = function() {
      // Show the info popup
      document.getElementById('info-popup').style.display = 'block';
    };

    // Add the 'Info' button to the GUI
    gui.add(params, 'showInfo').name('Information');
...
gui.add(params, 'showGrid').name('Show Grid').onChange(() => {
	  setModeParameters();
      recreateObjects();
	});
	gui.add(params, 'showArrows').name('Show Arrows').onChange(() => {
...
gui.add(params, 'maxKernelAngle', 0, 360.0).name('Max Winding');
...
function setModeParameters() {
  ...
  gridMeshes.forEach(mesh => {
    // Make sure originalPositions is properly set for each mesh
    let originalPositions = mesh.userData.originalPositions;

    // Loop through all vertices of the mesh
    const vertexCount = mesh.userData.vertexCount;
    ...
    for (let i = 0; i < vertexCount; i++) {
      ...
      const direction = position.clone().sub(new THREE.Vector3(0, 0, 0)).normalize();

      const arrowLength = 0.5; // Adjust length as needed
      const color = 0xff0000; 

      const arrowHelper = new THREE.ArrowHelper(direction, position, arrowLength, color);
      arrowHelper.userData.originalPosition = position.clone();
      arrowHelper.userData.originalDirection = direction.clone();
...
function removeArrows() {
  ...
  arrows = [];
    ...
    function createObjects() {
      ...
      createMarkers();
	    createCenterSphere(); // Add this line
    }
...
function createGrid() {
  ...
  for (let i = 0; i < numLayers; i++) {
	...
	gridMesh.visible = params.showGrid;
...
function animate() {
  ...
  // Update arrows if visible
  if (params.showArrows) {
    arrows.forEach((arrow) => {
      const originalPosition = arrow.userData.originalPosition;
      ...
      const newDirection = originalDirection.clone().applyQuaternion(combinedQuaternion).normalize();
